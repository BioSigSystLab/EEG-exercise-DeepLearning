...
for c=1,#classes do
    ...
    for t=1,classSize[c]/batchSize do
        local input1 = torch.CudaTensor(batchSize,3,64,64)
        local input2 = torch.CudaTensor(batchSize,3,64,64)
    ...
    if classSize[c]%batchSize~=0 then
        ...
        local input1 = torch.Tensor(remaining,3,64,64)
        local input2 = torch.Tensor(remaining,3,64,64)
