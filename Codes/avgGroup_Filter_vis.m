...
for f=1:12;
    ...
    %Bias1  = zeros(6,1);
    % 8 9 13 18 19 43 47 48
    % 10 11 15 20 21 44 49 50 54
    % 6 35 39 40
    % 4 34 37 38
    % 26 58 59 63
    % 24 56 57 61
    A = mean(mean(squeeze(TF([24 56 57 61],2501:10000,:)))); 
    A=reshape(A,[55,1]);
    B1 = max(0,conv(A,y1,'same')+Bias1(1));Y = [B1];
    B2 = max(0,conv(A,y2,'same')+Bias1(2));Y = [Y B2];
    B3 = max(0,conv(A,y3,'same')+Bias1(3));Y = [Y B3];
    B4 = max(0,conv(A,y4,'same')+Bias1(4));Y = [Y B4];
    B5 = max(0,conv(A,y5,'same')+Bias1(5));Y = [Y B5];
    B6 = max(0,conv(A,y6,'same')+Bias1(6));Y = [Y B6];
    Y = Y';
    K = zeros(16,6,5);
    ...
    for i=1:size(out_GAP1,2);
        if abs(out_GAP1(1,i))>=thresh_level*max(abs(out_GAP1))
            thresh1(i,1)=1;
        end
        if abs(out_GAP2(1,i))>=thresh_level*max(abs(out_GAP2))
            thresh2(i,1)=1;
        end
    end
    thresh1 = thresh1.*A;
    ...
    if f==1
        ...
        inp = inp+A;
        activation = activation+out_GAP2;
    end
    
end
inp = inp/12; activation = activation/12;
figure(1);hold off;
plot(inp,'LineWidth',3);hold on;
thresh = zeros(55,1);
thresh_level=0.5;
for i=1:size(activation,2);
    if abs(activation(1,i))>=thresh_level*max(abs(activation))
        thresh(i,1)=1;
    end
end
thresh = thresh.*inp;
plot(thresh,'LineWidth',3);
%plot(activation','LineWidth',3);
legend('input','Thresholded output');